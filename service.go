...
func NewWithLoader(loader Loader, opts ...Option) (*Service, error) {
	...
	if len(langs) == 0 {
		...
		if el, ok := loader.(interface{ LanguagesErr() error }); ok {
			if langErr := el.LanguagesErr(); langErr != nil {
				return nil, corelog.E("i18n.NewWithLoader", "no languages available", langErr)
			}
		}
	...
	for _, lang := range langs {
		...
		if err != nil {
			return nil, corelog.E("i18n.NewWithLoader", fmt.Sprintf("failed to load locale %q", lang), err)
		}
...
func Default() *Service {
	...
	if err := Init(); err != nil {
		corelog.Error("i18n: failed to initialise default service", "err", err)
	}
...
func (s *Service) SetLanguage(lang string) error {
	...
	if err != nil {
		return corelog.E("i18n.Service.SetLanguage", fmt.Sprintf("invalid language tag %q", lang), err)
	}
	...
	if confidence == language.No {
		return corelog.E("i18n.Service.SetLanguage", fmt.Sprintf("unsupported language: %q", lang), nil)
	}
...
func (s *Service) AddLoader(loader Loader) error {
	...
	for _, lang := range langs {
		messages, grammar, err := loader.Load(lang)
		if err != nil {
			return corelog.E("i18n.Service.AddLoader", fmt.Sprintf("failed to load locale %q", lang), err)
		}
...
func (s *Service) LoadFS(fsys fs.FS, dir string) error {
	...
	if err != nil {
		return corelog.E("i18n.Service.LoadFS", "failed to read locales directory", err)
	}
	for _, entry := range entries {
		...
		if err != nil {
			return corelog.E("i18n.Service.LoadFS", fmt.Sprintf("failed to read locale %q", entry.Name()), err)
		}
		lang := strings.TrimSuffix(entry.Name(), ".json")
		lang = strings.ReplaceAll(lang, "_", "-")
		if err := s.loadJSON(lang, data); err != nil {
			return corelog.E("i18n.Service.LoadFS", fmt.Sprintf("failed to parse locale %q", entry.Name()), err)
		}
		tag := language.Make(lang)
