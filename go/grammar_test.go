...
func (regionFallbackLoader) Load(lang string) core.Result {
...
func TestPluralize_UsesLocaleSingularOverride(t *testing.T) {
	...
	svc, err := serviceFromResult(NewWithLoader(pluralizeOverrideLoader{}))
	if err != nil {
...
func (pluralizeOverrideLoader) Load(lang string) core.Result {
	grammar := &GrammarData{
		Nouns: map[string]NounForms{
	...
	return localeLoadResult(map[string]Message{}, grammar)
}
...
func TestGrammarFallbackToBaseLanguageTag(t *testing.T) {
	...
	svc, err := serviceFromResult(NewWithLoader(regionFallbackLoader{}))
	if err != nil {
		t.Fatalf("NewWithLoader() failed: %v", err)
	}
	SetDefault(svc)
...
func TestTemplateFuncs_NumericAlias(t *testing.T) {
	...
	got := buf.String()
	if !core.HasPrefix(got, "1,234,567|3 hours ago") {
...
func TestTemplateFuncs_NumericDirectAliases(t *testing.T) {
	...
	got := buf.String()
	if !core.HasPrefix(got, "1,234,567|3.14|85%|1.46 MB|3rd|3 hours ago") {
...
func TestTemplateFuncs_TimeHelpers(t *testing.T) {
	...
	got := buf.String()
	if !core.HasPrefix(got, "3 hours ago|") {
		t.Fatalf("template time helpers prefix = %q, want %q", got, "3 hours ago|")
	}
	if !core.Contains(got, "minutes ago") && !core.Contains(got, "just now") {
...
func TestGrammar_DualClassNouns_Bad(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		t.Log("DualClassNouns Bad edge case")
		got := DualClassNouns()
		delete(got, "build")
		_ = DualClassNouns()
	})
...
func TestGrammar_DualClassNouns_Ugly(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		t.Log("DualClassNouns Ugly edge case")
		got := DualClassNouns()
		if len(got) == 0 {
			t.Fatal("expected nouns")
		}
