...
func (l *FSLoader) Load(lang string) core.Result {
	...
	for _, filename := range variants {
		filePath := fsLocalePath(l.dir, filename)
	...
	if err != nil {
		...
		return failResult(golog.E("FSLoader.Load", "locale not found: "+lang, err))
	}

	var raw map[string]any
	if r := core.JSONUnmarshal(data, &raw); !r.OK {
		return failResult(golog.E("FSLoader.Load", "invalid JSON in locale: "+lang, core.NewError(r.Error())))
	}
	...
	return localeLoadResult(messages, grammar)
}

func fsLocalePath(dir, filename string) string {
	dir = core.TrimSuffix(core.Trim(dir), "/")
	if dir == "" || dir == "." {
		return filename
	}
	return dir + "/" + filename
...
func (l *FSLoader) Languages() []string {
	l.langOnce.Do(func() {
		...
		if err != nil {
			l.langErr = golog.E("FSLoader.Languages", "read locale directory: "+l.dir, err)
			return
		}
...
func (l *FSLoader) LanguagesErr() core.Result {
	l.Languages()
	if l.langErr != nil {
		return core.Fail(l.langErr)
	}
	return core.Ok(nil)
}
