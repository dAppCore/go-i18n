...
func TestFSLoaderLoad(t *testing.T) {
	loader := NewFSLoader(localeFS, "locales")
	messages, grammar, loadResult := localeFromResult(loader.Load("en"))
	if !loadResult.OK {
		t.Fatalf("Load(en) error: %v", loadResult.Error())
	}
...
func TestFSLoaderLoadMissing(t *testing.T) {
	loader := NewFSLoader(localeFS, "locales")
	_, _, loadResult := localeFromResult(loader.Load("xx"))
	if loadResult.OK {
...
func TestFSLoaderLoadFallsBackToBaseLanguage(t *testing.T) {
	...
	loader := NewFSLoader(fs, "locales")
	messages, grammar, loadResult := localeFromResult(loader.Load("en-GB"))
	if !loadResult.OK {
		t.Fatalf("Load(en-GB) error: %v", loadResult.Error())
	}
...
func TestFSLoaderLoadUsesCanonicalVariant(t *testing.T) {
	...
	loader := NewFSLoader(fs, "locales")
	messages, _, loadResult := localeFromResult(loader.Load("en-us"))
	if !loadResult.OK {
		t.Fatalf("Load(en-us) error: %v", loadResult.Error())
	}
	if got := messages["greeting"].Text; got != "hello" {
...
func TestNewWithLoader_LoadsGrammarOnlyLocale(t *testing.T) {
	...
	svc, err := serviceFromResult(NewWithLoader(NewFSLoader(loaderFS, ".")))
	if err != nil {
	...
	if err := errorFromResult(svc.SetLanguage("fr")); err != nil {
...
func TestNewWithLoader_AcceptsGrammarAliases(t *testing.T) {
	...
	if err := errorFromResult(svc.SetLanguage("en")); err != nil {
...
func TestFSLoaderLanguagesErr_Good(t *testing.T) {
	loader := NewFSLoader(localeFS, "locales")
	if err := errorFromResult(loader.LanguagesErr()); err != nil {
...
func TestCustomFSLoader(t *testing.T) {
	...
	svc, err := serviceFromResult(NewWithFS(fs, "locales", WithFallback("test")))
	if err != nil {
...
func TestCustomFSLoaderPreservesZeroSignalPriors(t *testing.T) {
	...
	loader := NewFSLoader(fs, "locales")
	_, grammar, loadResult := localeFromResult(loader.Load("test"))
	if !loadResult.OK {
		t.Fatalf("Load(test) failed: %v", loadResult.Error())
	}
	if grammar == nil {
...
func TestLoader_NewFSLoader_Good(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		loader := NewFSLoader(testLocaleFS(), "locales")
		langs := loader.Languages()
		if len(langs) == 0 {
...
func TestLoader_NewFSLoader_Bad(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		loader := NewFSLoader(testLocaleFS(), "missing")
		langs := loader.Languages()
		if len(langs) != 0 {
...
func TestLoader_NewFSLoader_Ugly(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		loader := NewFSLoader(testLocaleFS(), "")
		_ = loader.Languages()
	})
	if !called {
		t.Fatal("NewFSLoader was not exercised")
...
func TestLoader_FSLoader_Load_Good(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		loader := NewFSLoader(testLocaleFS(), "locales")
		messages, _, loadResult := localeFromResult(loader.Load("en"))
		if !loadResult.OK {
			t.Fatalf("Load(en) error: %v", loadResult.Error())
		}
		if len(messages) == 0 {
...
func TestLoader_FSLoader_Load_Bad(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		loader := NewFSLoader(testLocaleFS(), "locales")
		_, _, loadResult := localeFromResult(loader.Load("zz"))
		if loadResult.OK {
...
func TestLoader_FSLoader_Load_Ugly(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		loader := NewFSLoader(testLocaleFS(), "locales")
		_, _, loadResult := localeFromResult(loader.Load(""))
		if loadResult.OK {
