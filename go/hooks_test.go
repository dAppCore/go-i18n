...
func TestSetDefault_Good_LoadsRegisteredLocalesIntoFreshService(t *testing.T) {
	...
	first, err := serviceFromResult(New())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	second, err := serviceFromResult(New())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestInit_LoadsRegisteredLocales(t *testing.T) {
	...
	RegisterLocales(fs, "locales")
	if err := errorFromResult(Init()); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestNewCoreService_InvalidLanguagePreservesSetLanguageError(t *testing.T) {
	...
	if core.Contains(msg, "invalid language") {
...
func TestCoreService_DelegatesToWrappedService(t *testing.T) {
	...
	if err := errorFromResult(coreSvc.SetLanguage("en")); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if err := errorFromResult(coreSvc.LoadFS(fstest.MapFS{
...
func TestInit_ReDetectsRegisteredLocales(t *testing.T) {
	...
	if !core.Contains(svc.Language(), "de") {
...
func TestOnMissingKey_Good(t *testing.T) {
	...
	if ("hooks_test.go") != (core.PathBase(captured.CallerFile)) {
		t.Fatalf("want %v, got %v", "hooks_test.go", core.PathBase(captured.CallerFile))
	}
}
...
func TestHooks_RegisterLocales_Good(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		...
		RegisterLocales(testLocaleFS(), "locales")
...
func TestHooks_RegisterLocales_Bad(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		RegisterLocales(testLocaleFS(), "missing")
...
func TestHooks_RegisterLocaleProvider_Good(t *testing.T) {
	...
	noPanicForAudit(t, func() {
		called = true
		setDefaultForAudit(t)
		RegisterLocaleProvider(NewFSLoader(testLocaleFS(), "locales"))
		if len(Default().AvailableLanguages()) == 0 {
			t.Fatal("expected languages")
		}
	})
