...
func BuildReferences(tokeniser *Tokeniser, samples []ClassifiedText) core.Result {
	if len(samples) == 0 {
		return failResult(golog.E("BuildReferences", "empty sample set", nil))
	}
	...
	if len(grouped) == 0 {
		return failResult(golog.E("BuildReferences", "no samples with domain labels", nil))
	}
