...
func RegisterLocales(fsys fs.FS, dir string) {
	...
	if svc != nil {
		if r := svc.LoadFS(fsys, dir); !r.OK {
			golog.Error("i18n: RegisterLocales failed to load", "dir", dir, "err", r.Error())
		} else {
...
func loadRegisteredLocales(svc *Service) {
	...
	for _, reg := range locales {
		...
		if r := svc.LoadFS(reg.fsys, reg.dir); !r.OK {
			golog.Error("i18n: loadRegisteredLocales failed to load", "dir", reg.dir, "err", r.Error())
			continue
		}
		svc.markLocaleRegistrationLoaded(reg.id)
	}
...
func loadLocaleProvider(svc *Service, provider localeProviderRegistration) {
	...
	case Loader:
		if r := svc.AddLoader(p); !r.OK {
			golog.Error("i18n: loadLocaleProvider failed to add loader", "err", r.Error())
		}
	case interface{ LocaleSources() []FSSource }:
		for _, src := range p.LocaleSources() {
			if r := svc.LoadFS(src.FS, src.Dir); !r.OK {
				golog.Error("i18n: loadLocaleProvider failed to load", "dir", src.Dir, "err", r.Error())
			}
		}
	case interface {
		Available() []string
	...
	default:
		golog.Error("i18n: unsupported locale provider", "type", core.Sprintf("%T", provider.provider))
	}
...
func loadLocaleProviderBytes(svc *Service, provider interface {
	...
	for _, rawLang := range provider.Available() {
		...
		if err != nil {
			golog.Error("i18n: loadLocaleProviderBytes failed to load", "lang", rawLang, "err", err)
			continue
		}
		if r := svc.loadJSON(lang, data); !r.OK {
			golog.Error("i18n: loadLocaleProviderBytes failed to parse", "lang", lang, "err", r.Error())
		}
	}
