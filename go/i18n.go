...
//
//	r := SetLanguage("fr")
func SetLanguage(lang string) core.Result {
	return defaultServiceValue(core.Fail(ErrServiceNotInitialised), func(svc *Service) core.Result {
...
//	N("percent", 0.85)     // "85%"
//	N(byteUnitName(), 1536000) // "1.46 MB"
//	N("ordinal", 1)        // "1st"
//
...
func LoadFS(fsys fs.FS, dir string) {
	withDefaultService(func(svc *Service) {
		if r := svc.AddLoader(NewFSLoader(fsys, dir)); !r.OK {
			golog.Error("i18n: LoadFS failed", "dir", dir, "err", r.Error())
		}
	})
}
